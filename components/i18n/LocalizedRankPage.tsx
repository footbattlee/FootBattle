...
const copy = {
  tr: {
    ...
    board: "Sezon LP Sıralaması", mobileTitle: "Sıralama", mobileDesc: "Sezonun en yüksek LP puanlı oyuncuları.", loading: "Yükleniyor...", error: "Rank sıralaması yüklenemedi.", games: "oyun", win: "G", loss: "M",
  },
  en: {
    ...
    board: "Season LP Leaderboard", mobileTitle: "Rankings", mobileDesc: "Players with the highest LP this season.", loading: "Loading...", error: "Rank leaderboard could not be loaded.", games: "games", win: "W", loss: "L",
  },
} as const;
      ...
      {loading ? <div className="p-10 text-center text-slate-500">{t.loading}</div> : !data?.ok ? <div className="p-8 text-center text-red-300">{data?.error ?? t.error}</div> : (
        <div className="divide-y divide-white/[0.06]">
          {(data.leaderboard ?? []).map((entry) => (
            <div key={entry.userId} className="grid grid-cols-[32px_46px_minmax(0,1fr)_auto] items-center gap-2 px-3 py-3 sm:grid-cols-[42px_56px_minmax(0,1fr)_auto] sm:gap-4 sm:px-6">
              <div className="text-center text-sm font-black sm:text-base">#{entry.position}</div>
              <Image src={entry.rankIcon} alt={rankName(entry.rankName, locale)} width={56} height={56} className="h-11 w-11 object-contain sm:h-14 sm:w-14" />
              <div className="min-w-0"><p className="truncate text-sm font-black sm:text-base">{entry.displayName}</p><p className="truncate text-[10px] text-slate-500 sm:text-xs">{rankName(entry.rankName, locale)} · {entry.gamesPlayed} {t.games} · {entry.wins}{t.win}/{entry.losses}{t.loss}</p></div>
              <div className="text-right"><p className="text-sm font-black text-yellow-300 sm:text-base">{nf.format(entry.lp)}</p><p className="text-[9px] font-black text-slate-600 sm:text-[10px]">LP</p></div>
            </div>
          ))}
        </div>
      )}
