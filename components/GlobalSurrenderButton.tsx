  ...
  async function surrender() {
    ...
    const confirmed = window.confirm(
      daily
        ...
        : "Pes edersen oyun kayıp olarak tamamlanacak ve doğru cevap gösterilecek. Emin misin?",
    );
    ...
    try {
      ...
      if (!response.ok || !json?.ok) {
        throw new Error(json?.error ?? "Oyun sonlandırılamadı.");
      }
    ...
    <>
      {!result && (
        <div className="fixed bottom-[calc(18px+env(safe-area-inset-bottom))] left-4 z-[70]">
          ...
          {error && (
            <p className="mt-2 max-w-[260px] rounded-lg bg-red-950/95 px-3 py-2 text-[11px] font-semibold text-red-200 shadow-xl">
              {error}
            </p>
          )}
        </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-3xl border border-red-500/25 bg-[#0b1422] p-6 text-center text-white shadow-2xl">
            ...
            <p className="mt-2 text-sm text-slate-400">
              {config.label} kayıp olarak tamamlandı.
            </p>
            ...
            <div className="mt-6 flex flex-col gap-2">
              {daily && result.nextHref ? (
                <button
                  type="button"
                  onClick={() => router.push(result.nextHref!)}
                ...
                <button
                  type="button"
                  onClick={() => router.push("/")}
                  className="rounded-xl bg-white px-5 py-3 text-sm font-black text-[#07111f] transition hover:bg-slate-200"
                >
                  Ana Sayfaya Dön
                </button>
              )}
