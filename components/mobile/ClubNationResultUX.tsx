...
export default function ClubNationResultUX() {
  ...
  async function requestRematch() {
    ...
    try {
      ...
      setRematch(result);
      if (result.state === "accepted" && result.token) scheduleStart(result);
      else setMessage("Rövanş isteği gönderildi. Rakibin onayı bekleniyor.");
    } catch (error) {
  ...
  async function respondRematch(action: "accept" | "decline") {
    ...
    try {
      ...
      setRematch(result);
      if (action === "accept" && result.token) scheduleStart(result);
      else setMessage("Rövanş isteği reddedildi.");
    } catch (error) {
  ...
  async function shareResult() {
    ...
    try {
      if (navigator.share) await navigator.share({ title: "1 Takım 1 Millet · FootBattle", text, url });
    ...
    <>
      ...
      <div className="fixed inset-x-3 bottom-[calc(14px+env(safe-area-inset-bottom))] z-[80] mx-auto max-w-xl rounded-2xl border border-white/15 bg-[#101c2c]/95 p-2 shadow-2xl backdrop-blur-xl md:bottom-5">
        ...
        <div className="grid grid-cols-2 gap-2">
          <button type="button" data-rematch-consent-action="true" onClick={() => void requestRematch()} disabled={busy || outgoing} className="min-h-12 rounded-xl bg-yellow-400 px-3 font-black text-[#07111f] disabled:opacity-50">{busy ? "Hazırlanıyor..." : "🔁 Rövanş"}</button>
