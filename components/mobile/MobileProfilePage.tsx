    ...
    void (async () => {
      try {
        ...
        const { data } = await supabase
          .from("profiles")
          .select("username,display_name,avatar_url,total_score,current_streak,games_played,games_won")
          .eq("id", user.id)
          .maybeSingle();
        ...
        <section className="mt-4 rounded-3xl border border-white/10 bg-white/[0.035] p-4">
          <div className="grid grid-cols-[64px_minmax(0,1fr)_92px] items-center gap-3">
            {profile.avatar_url ? (
              ...
              <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-green-500/10 text-xl font-black text-green-200">{initials(displayName)}</div>
            )}
            ...
            {rank ? (
              <Link href={`/${locale}/rank`} className="rounded-2xl border border-yellow-300/15 bg-yellow-300/[0.05] p-2 text-center">
                <img src={rank.rankIcon} alt={rank.rankName} className="mx-auto h-10 w-10 object-contain" />
                <p className="mt-1 truncate text-[10px] font-black text-yellow-200">{rank.rankName.replace(" III", "")}</p>
                <p className="mt-0.5 text-[9px] font-black text-green-300">{nf.format(rank.lp)} LP</p>
              </Link>
            ) : <div />}
          </div>
        ...
        <section className="mt-4">
          ...
          <div className="mt-3 space-y-2">
            {friends.length ? friends.slice(0, 8).map((item) => (
              <article key={item.friendshipId} className="flex items-center gap-3 rounded-2xl border border-white/[0.08] bg-white/[0.03] px-3 py-3">
                <div className="relative">
                  {item.user.avatarUrl ? <img src={item.user.avatarUrl} alt="" className="h-10 w-10 rounded-xl object-cover" /> : <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-green-500/10 text-xs font-black text-green-200">{initials(item.user.displayName)}</div>}
                  <span className={`absolute -bottom-1 -right-1 h-3 w-3 rounded-full border-2 border-[#07111f] ${item.user.online ? "bg-green-400" : "bg-slate-600"}`} />
                </div>
                <div className="min-w-0 flex-1"><p className="truncate text-sm font-black">{item.user.displayName}</p><p className="mt-0.5 truncate text-[10px] text-slate-500">{item.user.username ? `@${item.user.username}` : item.user.lastSeenText || (tr ? "FootBattle oyuncusu" : "FootBattle player")}</p></div>
                <span className="text-[10px] font-black text-slate-400">{nf.format(Number(item.user.totalScore ?? 0))}</span>
              </article>
            )) : (
