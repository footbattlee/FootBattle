...
function isHalisahaShareFlow() {
  return (
    ...
    window.location.pathname.startsWith("/halisaha-kadro")
  );
}
    ...
    if (originalShare) {
      try {
        navigator.share = async (data: ShareData) => {
          // Halısaha has its own share pipeline: live pitch PNG + one short /share/ link.
          // Do not inject the generic UTM URL and do not replace its PNG with the
          // global FootBattle social card.
    ...
    if (clipboard && originalWriteText) {
      try {
        clipboard.writeText = async (text: string) => {
          // Halısaha copy/share must stay exactly as generated by its own flow.
          if (isHalisahaShareFlow()) {
