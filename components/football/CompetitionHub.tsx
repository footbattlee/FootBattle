...
function GameSuggestions({ locale, competition }: { locale: Locale; competition: CompetitionKey }) {
  ...
  const primaryTitle = competition === "super-lig"
    ? (tr ? "Süper Lig Futbolcuyu Tahmin Et" : "Süper Lig Guess the Player")
    : "Guess the Player";
  ...
  return (
    <section className="rounded-2xl border border-emerald-400/20 bg-emerald-400/[.08] p-5">
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-2">
        <Link href={primaryHref} className="rounded-xl bg-emerald-400 px-4 py-3 text-center text-xs font-black text-[#07111f] transition hover:bg-emerald-300">
          ⚽ {primaryTitle}
        </Link>
        <Link href={`/${locale}/games`} className="rounded-xl border border-white/10 bg-white/[.04] px-4 py-3 text-center text-xs font-black text-white transition hover:bg-white/[.08]">
          🎮 {tr ? "Tüm Oyunları Gör" : "See All Games"}
        </Link>
      </div>
    </section>
  );
