...
export default function CompetitionSeoContent({ competition, locale, snapshot }: { competition: CompetitionKey; locale: Locale; snapshot: CompetitionSnapshot }) {
  ...
  const faqs = tr ? [
    ...
    { question: `${name} fikstürü ve maç sonuçları nerede?`, answer: `Yaklaşan maçlar ve tamamlanan karşılaşmalar hafta veya tur seçerek FootBattle üzerinden görüntülenebilir. Maç saatleri Türkiye saatine göre gösterilir.` },
    { question: `${name} maçlarına skor tahmini yapılabilir mi?`, answer: `Evet. Giriş yapan FootBattle kullanıcıları maç başlamadan önce skor tahmini yapabilir ve tahminlerini başlangıç saatine kadar değiştirebilir.` },
    ...(superLig ? [{ question: "Süper Lig takımlarının fikstür ve kadroları nerede?", answer: "Puan durumundaki bir takıma dokunarak o kulübün 2026/27 fikstürünü, maç sonuçlarını, organizasyonlarını ve güncel oyuncu kadrosunu görebilirsin." }] : []),
  ] : [
    { question: `Where can I follow the ${name} standings?`, answer: `The FootBattle ${name} hub shows the current 2026/27 standings, matches played and points in one place.` },
    { question: `Where can I see ${name} fixtures and results?`, answer: `Upcoming fixtures and completed matches can be browsed by matchweek or round on FootBattle. Match times are displayed in Türkiye time.` },
    { question: `Can I predict ${name} match scores?`, answer: `Yes. Signed-in FootBattle users can submit score predictions before kickoff and edit them until the match starts.` },
  ];

  const graph = { "@context": "https://schema.org", "@graph": [
    { "@type": "CollectionPage", "@id": `${url}#page`, url, name: tr ? `${name} Puan Durumu, Fikstür ve Sonuçlar` : `${name} Standings, Fixtures and Results`, description: tr ? `${name} 2026/27 puan durumu, fikstür, maç sonuçları, takım sayfaları ve skor tahminleri.` : `${name} 2026/27 standings, fixtures, results, team pages and score predictions.`, inLanguage: tr ? "tr-TR" : "en-US", isPartOf: { "@id": `${SITE_URL}/#website` }, about: { "@id": `${url}#competition` } },
    { "@type": "SportsOrganization", "@id": `${url}#competition`, name, sport: "Football", url, member: snapshot.standings.slice(0, 40).map((team) => ({ "@type": "SportsTeam", name: team.teamName, ...(team.logo ? { logo: team.logo } : {}), url: `${SITE_URL}/${locale}/${competition}/team/${team.teamId}` })) },
