...
const PERSISTED_GRID_POOL_SIZE = 40;
...
export function duelScore(correctCount: number) {
  return correctCount * TIC_TAC_TOE_SCORE_PER_CORRECT + (correctCount >= 9 ? TIC_TAC_TOE_FULL_GRID_BONUS : 0);
}
...
function validPersistedGrid(row: { rows?: unknown; columns?: unknown; grid_cells?: unknown }) {
  return Array.isArray(row.rows) && row.rows.length === 3 && Array.isArray(row.columns) && row.columns.length === 3 && Array.isArray(row.grid_cells) && row.grid_cells.length === 9;
}

/**
 * Existing duel grids are already quality-checked by generateBalancedTicTacToeGrid.
 * Reusing one turns historical duel rows into a persistent grid pool, so a Vercel
 * cold start does not need to rescan player/team history before every new match.
 */
async function loadPersistedGridTemplate() {
  const { data, error } = await supabaseAdmin
    .from("tic_tac_toe_duels")
    .select("rows, columns, grid_cells")
    .not("grid_cells", "is", null)
    .order("started_at", { ascending: false })
    .limit(PERSISTED_GRID_POOL_SIZE);

  if (error) {
    console.error("Tic Tac Toe persisted grid pool read error:", error);
    return null;
  }

  const candidates = (data ?? []).filter(validPersistedGrid);
  if (!candidates.length) return null;
  const selected = candidates[Math.floor(Math.random() * candidates.length)];
  return {
    rows: selected.rows as StoredAxis[],
    columns: selected.columns as StoredAxis[],
    cells: selected.grid_cells as StoredGridCell[],
  };
}

export async function loadTicTacToeChallenge(token: string) {
  const { data, error } = await supabaseAdmin.from("guest_challenges").select(`
  ...
  return data ? (data as DuelChallenge) : null;
}
...
export async function requireTicTacToeParticipant(tokenValue: unknown) {
  ...
  if (challenge.game_code !== "tic_tac_toe") return { ok: false as const, status: 409, error: "Bu bağlantı Tic Tac Toe düellosu değil." };
  const identity = await resolveDuelRole(challenge);
  if (identity.role !== "challenger" && identity.role !== "opponent") return { ok: false as const, status: 403, error: "Bu düelloya erişim yetkin yok." };
  return { ok: true as const, token, challenge, role: identity.role, user: identity.user };
}

async function createSecuritySessions(duel: DuelRow, challenge: DuelChallenge) {
  const expiresAt = new Date(new Date(duel.started_at).getTime() + Number(duel.duration_seconds) * 1000).toISOString();
  const rows = (["challenger", "opponent"] as DuelSide[]).map((side) => ({
    ...
    user_id: side === "challenger" ? challenge.challenger_user_id : challenge.opponent_user_id,
    mode: "duel",
    status: "active",
    started_at: duel.started_at,
    expires_at: expiresAt,
    metadata: { challenge_id: challenge.id, invite_token: challenge.invite_token, side },
  }));
  const { error } = await supabaseAdmin.from("game_sessions").upsert(rows, { onConflict: "game_code,source_session_id", ignoreDuplicates: true });
  if (error) console.error("Tic Tac Toe duel security session create error:", error);
}

export async function ensureTicTacToeDuel(challenge: DuelChallenge) {
  const existingResult = await supabaseAdmin.from("tic_tac_toe_duels")
    ...
    .eq("challenge_id", challenge.id).maybeSingle();
  ...
  // Fast path: copy one already validated grid from the persistent duel pool.
  // Slow generator remains as a safe fallback for a brand-new/empty database.
  const template = await loadPersistedGridTemplate();
  let rows: StoredAxis[];
  let columns: StoredAxis[];
  let cells: StoredGridCell[];
  ...
  if (template) {
    ...
    if (!grid || grid.rows.length !== 3 || grid.columns.length !== 3 || grid.cells.length !== 9) throw new Error("Düello için geçerli Tic Tac Toe grid'i oluşturulamadı.");
    rows = grid.rows.map(axisFromGrid);
    columns = grid.columns.map(axisFromGrid);
  ...
  const { data, error } = await supabaseAdmin.from("tic_tac_toe_duels").insert({
    challenge_id: challenge.id, rows, columns, grid_cells: cells,
    duration_seconds: TIC_TAC_TOE_DUEL_DURATION_SECONDS, started_at: startedAt,
  }).select("id, challenge_id, rows, columns, grid_cells, duration_seconds, started_at, challenger_finished_at, opponent_finished_at, finalized_at").maybeSingle();

  if (error) {
    if ((error as { code?: string }).code === "23505") {
      const retry = await supabaseAdmin.from("tic_tac_toe_duels")
        .select("id, challenge_id, rows, columns, grid_cells, duration_seconds, started_at, challenger_finished_at, opponent_finished_at, finalized_at")
        .eq("challenge_id", challenge.id).single();
...
export async function getDuelAttempts(duelId: string) {
  const { data, error } = await supabaseAdmin.from("tic_tac_toe_duel_attempts")
    .select("id, duel_id, side, row_index, column_index, player_id, correct, answered_at")
    .eq("duel_id", duelId).order("answered_at", { ascending: true });
  if (error) throw error;
...
export function sideStats(attempts: DuelAttempt[], side: DuelSide) {
  ...
  return { correctCount: correct.length, wrongCount: mine.length - correct.length, score: duelScore(correct.length), correct };
}

function completionMs(duel: DuelRow, side: DuelSide) {
  const finish = side === "challenger" ? duel.challenger_finished_at : duel.opponent_finished_at;
  if (!finish) return Number.POSITIVE_INFINITY;
  return Math.max(0, new Date(finish).getTime() - new Date(duel.started_at).getTime());
}

export async function maybeFinalizeDuel(duel: DuelRow, challenge: DuelChallenge, attempts?: DuelAttempt[]) {
  ...
  if (challengerIdle !== opponentIdle) winner = challengerIdle ? "opponent" : "challenger";
  else if (challenger.score !== opponent.score) winner = challenger.score > opponent.score ? "challenger" : "opponent";
  else if (challenger.wrongCount !== opponent.wrongCount) winner = challenger.wrongCount < opponent.wrongCount ? "challenger" : "opponent";
  else {
    const challengerMs = completionMs(duel, "challenger");
    const opponentMs = completionMs(duel, "opponent");
    if (challengerMs !== opponentMs) winner = challengerMs < opponentMs ? "challenger" : "opponent";
  }

  const now = new Date().toISOString();
  const { data: updated, error } = await supabaseAdmin.from("guest_challenges").update({
    status: "completed", challenger_score: challenger.score, opponent_score: opponent.score,
    winner_side: winner, completed_at: now, updated_at: now,
  }).eq("id", challenge.id).neq("status", "completed").select(`
      id, invite_token, game_code, status,
      challenger_user_id, challenger_guest_id, challenger_name,
      opponent_user_id, opponent_guest_id, opponent_name,
      challenger_score, opponent_score, winner_side,
  ...
  await supabaseAdmin.from("tic_tac_toe_duels").update({ finalized_at: now, updated_at: now }).eq("id", duel.id).is("finalized_at", null);
  const elapsedMs = Math.max(0, Math.min(Number(duel.duration_seconds) * 1000, Date.now() - new Date(duel.started_at).getTime()));
  for (const side of ["challenger", "opponent"] as DuelSide[]) {
    ...
    await supabaseAdmin.from("game_sessions").update({
      status: "finished", finished_at: now, server_score: stats.score, won: winner === side,
      duration_ms: elapsedMs, updated_at: now,
    }).eq("game_code", "tic_tac_toe").eq("source_session_id", duelSourceSessionId(duel.id, side)).eq("status", "active");
  }
  return (updated ?? { ...challenge, status: "completed", challenger_score: challenger.score, opponent_score: opponent.score, winner_side: winner, completed_at: now }) as DuelChallenge;
}
