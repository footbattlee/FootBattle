...
export async function syncRankedMatchCompletion(
  token: string,
  ...
  await supabaseAdmin
    .from("ranked_matches")
    .update({ status: "completed", winner_user_id: winnerUserId, completed_at: now, updated_at: now })
    .eq("id", match.id)
...
export async function runRankedTicTacToeBotTick(challenge: DuelChallenge) {
  ...
  const { data: claimed, error: claimError } = await supabaseAdmin
    ...
    .update({ current_turn: "challenger", turn_started_at: now, draw_offer_by: null, updated_at: now })
    .eq("id", duel.id)
    .eq("current_turn", "opponent")
    .eq("turn_started_at", turn.turnStartedAt)
    .select("id")
    .maybeSingle();
  ...
  const occupied = new Set(
    attempts.filter((a) => a.correct).map((a) => `${Number(a.row_index)}:${Number(a.column_index)}`),
  );
  const emptyCells = duel.grid_cells.filter(
    (cell) => !occupied.has(`${Number(cell.rowIndex)}:${Number(cell.columnIndex)}`) && cell.validPlayerIds.length > 0,
  );
  ...
  const { error: insertError } = await supabaseAdmin
    ...
    .insert({
      duel_id: duel.id,
      side: "opponent",
      row_index: Number(cell.rowIndex),
      column_index: Number(cell.columnIndex),
      player_id: playerId,
      correct: true,
      answered_at: now,
    });
...
export async function runRankedClubClashBotTick(token: string) {
  ...
  const { data: challenge, error: challengeError } = await supabaseAdmin
    ...
    .select("id,status,started_at,winner_side")
  ...
  const { data: rounds, error: roundsError } = await supabaseAdmin
    ...
    .select("id,round_no,left_type,left_value,right_type,right_value,winner_side,completed_at,created_at")
  ...
  if (Date.now() - new Date(roundStartedAt).getTime() < stableDelayMs(`${match.id}:cc:${current.round_no}`, 4500, 6500)) return false;
  ...
  const { data: player } = await supabaseAdmin
    ...
    .maybeSingle();
  const answer = String(player?.name ?? "Mehmet'in cevabı");
  const now = new Date().toISOString();
  ...
  const { data: claimed, error: claimError } = await supabaseAdmin
    ...
    .update({
      ...
      opponent_answer_player_id: playerId,
  ...
  if (opponentScore >= 3) {
    await supabaseAdmin.from("guest_challenges").update({
      ...
      completed_at: now,
      updated_at: now,
    }).eq("id", challenge.id).neq("status", "completed");
    ...
    await supabaseAdmin.from("guest_challenges").update({
      ...
      opponent_score: opponentScore,
      updated_at: now,
    }).eq("id", challenge.id).eq("status", "playing");
  }
  return true;
}
