...
function ResultCard({ result, onRematch, onShare, busy }: { result: "win" | "loss" | "draw" | null; onRematch: () => void; onShare: () => void; busy: boolean }) {
  ...
  return <section className="mt-3 rounded-3xl border border-yellow-400/25 bg-yellow-400/[0.05] p-6 text-center"><div className="text-5xl">{result === "win" ? "🏆" : result === "loss" ? "🥈" : "🤝"}</div><h2 className="mt-3 text-3xl font-black">{title}</h2><p className="mt-2 text-sm text-slate-400">Final grid aşağıda. Kazandıran üçlü sarı çerçeveyle vurgulandı.</p><div className="mt-5 grid grid-cols-2 gap-2"><button disabled={busy} onClick={onRematch} className="rounded-2xl bg-yellow-400 px-4 py-3 font-black text-[#07111f] disabled:opacity-50">🔁 Rövanş</button><button disabled={busy} onClick={onShare} className="rounded-2xl border border-green-400/30 bg-green-400/10 px-4 py-3 font-black text-green-200 disabled:opacity-50">↗ Sonucu Paylaş</button><Link href="/" className="col-span-2 flex min-h-12 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3 font-black text-slate-200">⌂ Ana Sayfa</Link></div></section>;
}
