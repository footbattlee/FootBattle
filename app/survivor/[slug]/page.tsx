...
export default function SurvivorGamePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    void params.then(({ slug: resolved }) => setSlug(resolved));
  }, [params]);
  ...
  async function saveResult(nextChampion: Entry, nextHistory: Round[]) {
    ...
    try {
      ...
      // Oyun sonucu ekranda kalmaya devam eder; paylaşım normal oyun linkine düşer.
    }
  }
  ...
  async function share() {
    ...
    setSharing(true);
    const target = shareUrl ? `${window.location.origin}${shareUrl}` : `${window.location.origin}/survivor/${survivor.slug}?utm_source=share&utm_medium=survivor`;
    const text = `👑 Benim “${survivor.title}” şampiyonum ${champion.name}!\nSenin tahtın sahibi kim olacak?`;
    try {
  ...
  if (error || !survivor) return <main className="min-h-screen bg-[#07111f] p-8 text-white"><p className="text-red-300">{error || "Survivor bulunamadı."}</p><Link href="/survivor" className="mt-4 inline-block text-green-300">← Survivor listesi</Link></main>;
      ...
      <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-9">
        ...
        {!champion && currentMatch && (
          <section className="mt-7">
            ...
            <p className="mt-5 text-center text-xs font-bold text-slate-500">Turu geçen kişiyi seç. Seçimin geri alınamaz ve bracket artık değişmez.</p>
          </section>
        )}
