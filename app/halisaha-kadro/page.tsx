...
/*
 * ESKİ UZUN PAYLAŞIM LİNKLERİ
 *
 * Artık yeni link üretirken kullanılmıyor.
 * Fakat daha önce paylaşılmış ?kadro=... linkleri
 * bozulmasın diye decode tarafını koruyoruz.
 */
function decodeLegacySharePayload(
  value: string,
): SharePayload | null {
  try {
    return JSON.parse(
...
export default function HalisahaKadroPage() {
  ...
  /*
   * Kadro değiştiyse daha önce oluşturulan
   * kısa link artık eski kadroyu temsil eder.
   *
   * Bu nedenle linki temizliyoruz.
   * Kullanıcı tekrar Kopyala/Paylaş dediğinde
   * yeni snapshot oluşturulacak.
   */
  useEffect(() => {
    setShareUrl("");
    setCopyMessage("");
  ...
  /*
   * Legacy ?kadro=... linklerini açmaya devam et.
   */
  useEffect(() => {
    const params =
      new URLSearchParams(
  ...
  async function createShortShareUrl() {
    /*
     * Aynı kadro değişmeden daha önce link
     * oluşturulduysa yeni DB kaydı açmayalım.
     */
    if (shareUrl) {
      return shareUrl;
    }
    ...
    const result =
      (await response.json()) as ShareApiResponse;
  ...
  async function copyShareLink() {
    try {
      ...
      setCopyMessage(
        "Kısa paylaşım linki kopyalandı!",
      );
  ...
  /* =======================================================
     ...
     ÖNEMLİ:
     PNG dosyasını artık native share'e eklemiyoruz.

     Çünkü WhatsApp'ta URL'nin Open Graph preview'sunun
     oluşmasını istiyoruz.

     PNG indirme butonu ayrı olarak kalmaya devam ediyor.
  ======================================================= */
  ...
  async function shareSquad() {
    try {
      ...
      window.alert(
        "Tarayıcın doğrudan paylaşımı desteklemiyor. Kısa link panoya kopyalandı.",
      );
    } catch (error) {
      if (
        error instanceof
      ...
      window.alert(
        error instanceof Error
          ? error.message
          : "Paylaşım hazırlanırken bir hata oluştu.",
      );
    } finally {
      setShareLoading(
        false,
          ...
          <aside className="h-fit overflow-hidden rounded-3xl border border-white/10 bg-[#0d1828] shadow-2xl shadow-black/20 xl:sticky xl:top-4">
            ...
            <section className="border-b border-white/10 p-5 sm:p-6">
              ...
              <p className="mt-3 text-xs leading-5 text-slate-500">
                Taşı modunda oyuncuları sürükle. Ok veya kalem modunda sahaya basılı tutup çiz. Silgi modunda kaldırmak istediğin çizginin üzerine dokun.
              </p>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-400">
                    Kadron için kısa bir FootBattle bağlantısı oluştur ve arkadaşlarına gönder.
                  </p>
              ...
              <div className="mt-5 grid gap-4 lg:grid-cols-[220px_minmax(0,1fr)]">
                ...
                <div className="flex flex-col justify-center">
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row">
                    ...
                    <div className="flex min-w-0 flex-1 items-center gap-3 rounded-xl border border-white/10 bg-[#07111f] px-4 py-3">
                      ...
                      <input
                        ...
                        placeholder="Kopyala veya Paylaş dediğinde kısa link oluşturulur"
                    ...
                    <button
                      ...
                      className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border border-yellow-400/50 bg-yellow-400/10 px-5 py-3 font-black text-yellow-300 transition hover:bg-yellow-400/20 disabled:cursor-wait disabled:opacity-60"
                    >
                      <Copy
                        size={
                          17
                  ...
                  <button
                    ...
                    className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 px-5 py-4 font-black transition hover:bg-white/5 disabled:cursor-wait disabled:opacity-60"
                  >
                    <Share2
                      size={
                        19
  ...
  >(
    function FootballPitchComponent(
      ...
      const [
        ...
        useState<DrawingItem | null>(
          null,
        );

      function setCombinedRef(
        node:
          HTMLDivElement | null,
      ) {
        pitchContainerRef.current =
          node;
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{
                    background:
                      `linear-gradient(90deg, ${sleeveColor} 0 20%, ${bodyColor} 20% 80%, ${sleeveColor} 80% 100%)`,
                  }}
                />
              </div>
            );
