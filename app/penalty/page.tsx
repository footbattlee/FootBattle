...
export default function PenaltyPage() {
  ...
  const [keeperRotation, setKeeperRotation] = useState(0);
  ...
  const pulledBall = useMemo<Point>(() => {
    if (!dragging) return ball;
    return {
      x: Math.max(34, Math.min(66, BALL_START.x + dragOffset.x * 0.58)),
      y: Math.max(BALL_START.y, Math.min(82, BALL_START.y + dragOffset.y * 0.62)),
    };
  }, [ball, dragOffset, dragging]);
  ...
  function shoot() {
    ...
    const reactsCorrectly = Math.random() < 0.68;
    const keeperX = reactsCorrectly
      ? Math.max(29, Math.min(71, aim.x + (Math.random() - 0.5) * 14))
      : 30 + Math.random() * 40;
    const keeperY = reactsCorrectly
      ? Math.max(24, Math.min(31, aim.y + (Math.random() - 0.5) * 4))
      : 25 + Math.random() * 5;

    setKeeper({ x: keeperX, y: keeperY });
    setKeeperRotation(Math.max(-26, Math.min(26, (keeperX - 50) * 1.15)));
    setBall(aim);

    const horizontalGap = Math.abs(aim.x - keeperX);
    const verticalGap = Math.abs(aim.y - keeperY);
    const saved = horizontalGap <= 13.5 && verticalGap <= 7.5;
    ...
    <main className="min-h-screen overflow-x-hidden bg-[#06152b] text-white select-none">
      <div className="mx-auto max-w-md pb-[max(18px,env(safe-area-inset-bottom))]">
        <header className="flex items-center justify-between bg-[#071d3c] px-3 py-3">
          <Link href="/tr" aria-label="Ana sayfaya dön" className="flex items-center gap-2 rounded-xl px-1 py-1 active:scale-95">
        ...
        {!finished ? (
          ...
          >
            ...
            >
              <Keeper diving={result !== null && Math.abs(keeper.x - 50) > 7} />
            </div>
            ...
            <button
              ...
              className="absolute z-40 -translate-x-1/2 -translate-y-1/2 transition-all duration-150 active:scale-110"
              style={{ left: `${pulledBall.x}%`, top: `${pulledBall.y}%`, touchAction: "none" }}
            >
              <Football />
            </button>
            ...
            {result ? (
              <button
                onClick={next}
                className="absolute bottom-[4%] left-[17%] right-[17%] z-50 rounded-xl border border-[#0d3154] bg-gradient-to-b from-[#ffd642] to-[#ffb51e] py-3 text-[13px] font-black text-[#06152b] shadow-[0_4px_0_#0a2744] active:translate-y-0.5"
              >
...
function Keeper({ diving }: { diving: boolean }) {
  return (
    <div className={`relative drop-shadow-[0_3px_3px_rgba(0,0,0,.35)] ${diving ? "h-[54px] w-[92px]" : "h-[64px] w-[72px]"}`}>
      <div className="absolute left-1/2 top-0 h-[15px] w-[15px] -translate-x-1/2 rounded-full border border-[#2e231c] bg-[#b9784b]" />
      <div className="absolute left-1/2 top-[13px] h-[30px] w-[25px] -translate-x-1/2 rounded-t-[9px] bg-[#176b38]">
        <span className="absolute left-1/2 top-[6px] -translate-x-1/2 text-[8px] font-black text-white">1</span>
      </div>
      <div className="absolute left-[5px] top-[18px] h-[6px] w-[27px] origin-right -rotate-[12deg] rounded-full bg-[#176b38]" />
      <div className="absolute right-[5px] top-[18px] h-[6px] w-[27px] origin-left rotate-[12deg] rounded-full bg-[#176b38]" />
      <div className="absolute left-0 top-[14px] h-[10px] w-[9px] rounded bg-[#f4c430]" />
      <div className="absolute right-0 top-[14px] h-[10px] w-[9px] rounded bg-[#f4c430]" />
      <div className="absolute bottom-[4px] left-[23px] h-[19px] w-[7px] rounded bg-[#121b2c]" />
      <div className="absolute bottom-[4px] right-[23px] h-[19px] w-[7px] rounded bg-[#121b2c]" />
      <div className="absolute bottom-0 left-[17px] h-[4px] w-[18px] rounded-full bg-white" />
      <div className="absolute bottom-0 right-[17px] h-[4px] w-[18px] rounded-full bg-white" />
    </div>
  );
}
