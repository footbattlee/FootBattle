...
export default function TransferQuizPage() {
  ...
  const finishGame = useCallback(async (reason: "timeout" | "surrender" = "timeout") => {
    ...
    try {
      const response = await fetch("/api/transfer-quiz/result", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId, finishReason: reason }) });
      ...
      if (response.ok && result.ok) {
        ...
        setScore(finalScore); setCorrectCount(finalCorrect); setPassesUsed(finalPasses); setResultState("saved");
  ...
  if (finished) return (
    <Shell><div className="mx-auto max-w-xl rounded-[28px] border border-white/10 bg-white/[0.06] p-6 text-center shadow-2xl">
      ...
      <button onClick={() => void startGame(true)} className="mt-6 w-full rounded-2xl bg-emerald-400 px-5 py-4 text-lg font-black text-slate-950">Tekrar Oyna</button><Link href="/tr" className="mt-3 block rounded-2xl border border-white/10 px-5 py-4 font-bold text-white/70">Ana Sayfa</Link>
    </div></Shell>
  );

  return (
    <Shell><div className="mx-auto max-w-2xl">
      ...
      <section className="overflow-hidden rounded-[28px] border border-white/10 bg-white/[0.055] shadow-2xl">
        ...
        <div className="p-4 sm:p-6">
          ...
          <div className="mt-4 text-center">
            <p className="text-sm font-semibold text-white/45">BU TRANSFERİ KİM YAPTI?</p>
            <div className="mt-3 grid grid-cols-[1fr_auto_1fr] items-center gap-3 rounded-3xl border border-white/10 bg-black/20 p-4 sm:p-6"><Club name={question.fromClubName} label="ESKİ KULÜP"/><div className="text-2xl text-emerald-300">→</div><Club name={question.toClubName} label="YENİ KULÜP"/></div>
            <div className="mt-3 flex justify-center gap-3 text-sm"><span className="rounded-xl bg-emerald-400/10 px-3 py-2 font-black text-emerald-300">{formatFee(question.transferFee)}</span>{question.transferSeason && <span className="rounded-xl bg-white/5 px-3 py-2 font-bold text-white/60">{question.transferSeason}</span>}</div>
          </div>
          ...
          <div className="mt-2 min-h-6 text-center text-sm font-bold text-white/65">{feedback}</div>
          <div className="mt-2 grid grid-cols-2 gap-3"><button onClick={()=>void usePass()} disabled={busy||passesLeft<=0} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3.5 font-black text-white/75 disabled:opacity-35">PAS GEÇ · {passesLeft}</button><button onClick={()=>setShowSurrenderConfirm(true)} disabled={busy} className="rounded-2xl border border-rose-400/25 bg-rose-500/10 px-4 py-3.5 font-black text-rose-300 disabled:opacity-35">PES ET</button></div>
        </div>
      </section>
    </div>
