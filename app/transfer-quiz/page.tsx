...
export default function TransferQuizPage() {
      ...
      async (
        ...
        try {
          ...
          if (
            ...
            throw new Error(
              result.error ??
                "Yeni Transfer Quiz hazırlanamadı.",
            );
          }
          ...
          if (
            ...
            throw new Error(
              "Transfer Quiz bilgileri eksik geldi.",
            );
          }
          ...
          setMessage(
            initial
              ? "😏 Footy: İstediğin kutudan başlayabilirsin."
              : `🔥 Footy: Transfer Quiz yeniden başladı. ${session.player.fullName} seni bekliyor.`,
          );
        } catch (error) {
          console.error(
            "Transfer Quiz yükleme hatası:",
            error,
          );
      ...
      window.setTimeout(
        async () => {
          try {
            ...
            const response =
              await fetch(
                `/api/transfer-quiz/search-country?q=${encodeURIComponent(
                  query,
                )}`,
                {
                  cache:
      ...
      window.setTimeout(
        async () => {
          try {
            ...
            const response =
              await fetch(
                `/api/transfer-quiz/search-club?q=${encodeURIComponent(
                  query,
                )}`,
                {
                  cache:
      ...
      async (
        ...
        try {
          ...
          if (
            ...
            setResultSaveMessage(
              "Transfer Quiz sonucun kaydedildi.",
            );
          }
        } catch (error) {
          console.error(
            "Transfer Quiz sonuç kayıt hatası:",
            error,
          );
  ...
  async function submitField(
    ...
    try {
      ...
      console.error(
        "Transfer Quiz cevap kontrol hatası:",
        error,
      );
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mt-4 overflow-hidden rounded-2xl border border-yellow-400/20 bg-[#111b2a] shadow-2xl shadow-black/40 sm:mt-7 sm:rounded-3xl">
          ...
          <div className="border-b border-white/10 bg-gradient-to-b from-yellow-400/[0.11] to-transparent px-4 pb-4 pt-4 text-center sm:px-6 sm:pb-6 sm:pt-6">
            ...
            <div className="mx-auto mt-3 max-w-xl sm:mt-4">
              <p className="text-xs font-black uppercase tracking-[0.16em] text-yellow-300 sm:text-sm">
                {gameSession.headline}
              </p>

              {gameSession.targetClub && (
                <p className="mt-1 text-[11px] font-bold text-slate-400 sm:text-sm">
                  Transfer gündemi: {gameSession.targetClub}
                </p>
              )}
            </div>
            ...
            <p className="mx-auto mt-3 max-w-md text-xs leading-5 text-slate-400 sm:mt-4 sm:text-sm">
              Transfer gündemindeki yıldızı ne kadar tanıyorsun? Doğum yılını, uyruğunu ve A takım kariyer kulüplerini doldur.
            </p>
        ...
        <section className="mt-8 rounded-2xl border border-white/10 bg-white/[0.03] p-5 text-left">
          <h2 className="text-xl font-black text-white">
            Transfer Futbolcu Quiz
          </h2>

          <p className="mt-3 text-sm leading-6 text-slate-300">
            Transfer Futbolcu Quiz oyununda futbolcuların doğum yılı,
          ...
          <p className="mt-2 text-sm leading-6 text-slate-400">
            Test your football knowledge with transfer-focused player quizzes by guessing nationality, birth year and senior career clubs.
          </p>
