...
export default function GununKapismasiPage() {
  ...
  async function vote(choice: Choice) {
    ...
    try {
      ...
      setMessage(result.alreadyVoted ? "Oyunu daha önce kullanmışsın." : "Oy kaydedildi. Arena sonucu açıldı. 🔥");
    } catch (error) {
  ...
  return (
    <main className="min-h-screen bg-[#07111f] px-4 py-6 text-white sm:px-6 sm:py-10">
      <div className="mx-auto max-w-[900px]">
        ...
        {loading && (
          <div className="mt-10 rounded-3xl border border-white/10 bg-white/[0.03] p-10 text-center font-bold text-slate-400">Bugünün kapışması hazırlanıyor...</div>
        )}
          ...
          <section className="mt-8 overflow-hidden rounded-[32px] border border-white/10 bg-[#081523] shadow-2xl">
            ...
            <div className="relative grid md:grid-cols-2">
              <PlayerSide
                name={faceoff.left}
                side="left"
                selected={data.choice === "left"}
                disabled={Boolean(data.voted || voting)}
                loading={voting === "left"}
                percent={results?.leftPercent ?? null}
                votes={results?.left ?? null}
                onVote={vote}
              />
              ...
              <PlayerSide
                name={faceoff.right}
                side="right"
                selected={data.choice === "right"}
                disabled={Boolean(data.voted || voting)}
                loading={voting === "right"}
                percent={results?.rightPercent ?? null}
                votes={results?.right ?? null}
                onVote={vote}
              />
            </div>
        ...
        <div className="mt-7 rounded-2xl border border-white/10 bg-white/[0.025] p-4 text-center text-xs leading-5 text-slate-500">
          Oyunu kullandıktan sonra yüzdeler açılır. Aynı hesap veya cihaz bugünün kapışmasında yalnızca bir kez oy kullanabilir.
        </div>
      </div>
    </main>
  );
...
function PlayerSide({
  ...
  return (
    <button
      type="button"
      disabled={disabled}
      onClick={() => void onVote(side)}
      className={`group flex min-h-[300px] flex-col items-center justify-center p-8 text-center transition sm:min-h-[370px] ${side === "left" ? "bg-green-400/[0.035] hover:bg-green-400/[0.08]" : "bg-purple-400/[0.035] hover:bg-purple-400/[0.08]"} ${selected ? "ring-2 ring-inset ring-yellow-300/70" : ""} disabled:cursor-default`}
    >
      <div className={`flex h-28 w-28 items-center justify-center rounded-full border text-4xl font-black shadow-2xl sm:h-36 sm:w-36 sm:text-5xl ${side === "left" ? "border-green-400/30 bg-green-400/10 text-green-200" : "border-purple-400/30 bg-purple-400/10 text-purple-200"}`}>
        {initials(name)}
      </div>
