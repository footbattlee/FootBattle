...
type GameAnalyticsRow = { gameName: string; started: number; completed: number; abandoned: number; inProgress: number; playAgain: number; shared: number; uniqueUsers: number; completionRate: number; averageDurationSeconds: number };
...
type Audience = { dau: number; wau: number; uniquePlayers: number; newUsers: number; platforms: { web: number; android: number; unknown: number } };
...
const EMPTY_AUDIENCE: Audience = { dau:0,wau:0,uniquePlayers:0,newUsers:0,platforms:{web:0,android:0,unknown:0} };
...
export default function AdminAnalyticsPage(){
  ...
  return <main className="min-h-screen bg-[#07111f] px-4 py-8 text-white sm:px-6"><div className="mx-auto max-w-7xl">
    ...
    <section className="mt-8 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.025]"><div className="flex items-center justify-between border-b border-white/10 px-5 py-4"><div><p className="font-black">Solo Oyun Bazlı Lifecycle</p><p className="mt-1 text-xs text-slate-500">Source of truth: game_sessions. Rejected/security-blocked sonuçlar da oynanış olarak tamamlanmış sayılır; yalnız puan engellenir.</p></div><button onClick={()=>void loadAnalytics()} disabled={loading} className="rounded-xl border border-white/10 px-4 py-2 text-xs font-black">Yenile</button></div><div className="overflow-x-auto"><table className="w-full min-w-[980px] text-left"><thead><tr className="text-xs text-slate-500"><th className="p-4">Oyun</th><th>Tekil</th><th>Başlatma</th><th>Tamamlama</th><th>Terk</th><th>Aktif</th><th>Oran</th><th>Ort. Süre</th><th>Tekrar</th><th>Paylaşım</th></tr></thead><tbody>{games.map(g=><tr key={g.gameName} className="border-t border-white/5"><td className="p-4 font-black">{getGameLabel(g.gameName)}</td><td>{formatNumber(g.uniqueUsers)}</td><td>{formatNumber(g.started)}</td><td>{formatNumber(g.completed)}</td><td>{formatNumber(g.abandoned)}</td><td>{formatNumber(g.inProgress)}</td><td><CompletionBadge value={g.completionRate}/></td><td>{formatDuration(g.averageDurationSeconds)}</td><td>{formatNumber(g.playAgain)}</td><td>{formatNumber(g.shared)}</td></tr>)}</tbody></table></div></section>
