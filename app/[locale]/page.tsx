    ...
    <main className="min-h-screen bg-[#07111f] text-white">
      <div className="mx-auto max-w-7xl px-5 py-6 sm:px-6 lg:px-8">
        <nav className="flex items-center justify-between gap-4 border-b border-white/10 pb-5">
          <Link href={`/${locale}`} className="text-xl font-black tracking-tight text-green-300 sm:text-2xl">
            FootBattle
          </Link>
          ...
          <div className="flex items-center gap-2">
            <Link
              href="/rank"
              className="hidden rounded-xl border border-yellow-300/20 bg-yellow-300/[0.07] px-4 py-2 text-xs font-black text-yellow-100 transition hover:bg-yellow-300/10 sm:inline-flex"
            >
        ...
        <section className="relative overflow-hidden py-16 text-center sm:py-24 lg:py-28">
          ...
          <div className="relative">
            ...
            <h1 className="mx-auto mt-5 max-w-5xl text-4xl font-black tracking-tight sm:text-6xl lg:text-7xl">
              {dictionary.home.title}
            </h1>
            <p className="mx-auto mt-6 max-w-3xl text-sm leading-7 text-slate-400 sm:text-lg">
            ...
            <div className="mt-9 flex flex-col justify-center gap-3 sm:flex-row">
              <a
                href="#games"
                className="inline-flex min-h-12 items-center justify-center rounded-xl bg-green-500 px-7 text-sm font-black text-[#07111f] transition hover:bg-green-400"
              >
                ⚽ {dictionary.home.primaryCta}
              </a>
              <Link
                href="/rank"
                className="inline-flex min-h-12 items-center justify-center rounded-xl border border-yellow-300/25 bg-yellow-300/10 px-7 text-sm font-black text-yellow-100 transition hover:bg-yellow-300/15"
              >
                🏆 {dictionary.home.secondaryCta}
              </Link>
            </div>
        ...
        <section id="games" className="border-t border-white/10 py-12 sm:py-16">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.22em] text-green-300">FootBattle Games</p>
              <h2 className="mt-2 text-3xl font-black sm:text-4xl">{dictionary.home.gamesTitle}</h2>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">{dictionary.home.gamesSubtitle}</p>
            </div>
          </div>
          ...
          <div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {dictionary.home.games.map((game, index) => (
              <Link
                key={game.title}
                href={game.href}
                className="group rounded-3xl border border-white/10 bg-white/[0.035] p-5 transition hover:-translate-y-0.5 hover:border-green-400/30 hover:bg-white/[0.055]"
              >
                <div className="flex items-start justify-between gap-3">
                  <span className="flex h-11 w-11 items-center justify-center rounded-2xl border border-green-400/15 bg-green-400/[0.08] text-lg font-black text-green-200">
                    {index + 1}
                  </span>
                ...
                <h3 className="mt-5 text-xl font-black transition group-hover:text-green-200">{game.title}</h3>
                <p className="mt-2 text-sm leading-6 text-slate-400">{game.description}</p>
                <p className="mt-5 text-xs font-black text-green-300">{locale === "tr" ? "Oyna →" : "Play →"}</p>
              </Link>
            ))}
          </div>
        ...
        <section className="border-t border-white/10 py-12 sm:py-16">
          <div className="text-center">
            <p className="text-xs font-black uppercase tracking-[0.22em] text-yellow-300">FootBattle Account</p>
            <h2 className="mt-2 text-3xl font-black sm:text-4xl">{dictionary.home.featuresTitle}</h2>
            <p className="mx-auto mt-3 max-w-2xl text-sm leading-6 text-slate-400">{dictionary.home.featuresSubtitle}</p>
          </div>

          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {dictionary.home.features.map((feature, index) => (
              <article key={feature.title} className="rounded-3xl border border-white/10 bg-[#0b1726] p-6">
        ...
        <footer className="flex flex-col gap-3 border-t border-white/10 py-6 text-xs text-slate-600 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-4">
            <Link href="/rank" className="font-bold text-slate-400 hover:text-white">{dictionary.home.secondaryCta}</Link>
