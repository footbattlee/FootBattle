...
function displayDate(value: string, locale: "tr" | "en") {
...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const description = en
    ? `${faceoff.left_name} and ${faceoff.right_name} go head to head in ${faceoff.category}. Who would you pick in the FootBattle Daily Faceoff?`
    : `${faceoff.left_name} ve ${faceoff.right_name} karşı karşıya. ${faceoff.category} kapışmasında sen kimi seçerdin? FootBattle Günün Kapışması.`;
  return {
    title,
    description,
    alternates: {
...
export default async function LocalizedFaceoffDetailPage({ params }: { params: Params }) {
  ...
  const description = en
    ? `${faceoff.left_name} and ${faceoff.right_name} meet in the ${faceoff.category} category. Which side are you taking?`
    : `${faceoff.left_name} ve ${faceoff.right_name}, ${faceoff.category} kategorisinde karşı karşıya geliyor. Senin seçimin hangisi?`;
