...
function mapRound(round: RoundRow) {
  return {
    ...
    challenger: {
      answer: round.challenger_answer,
      playerId: round.challenger_answer_player_id,
      answeredAt: round.challenger_answered_at,
    },
    opponent: {
      answer: round.opponent_answer,
      playerId: round.opponent_answer_player_id,
      answeredAt: round.opponent_answered_at,
    },
  };
}

function calculateScore(rounds: RoundRow[]) {
  const completedRounds = rounds.filter((round) => Boolean(round.completed_at));
  return {
    challenger: completedRounds.filter((round) => round.winner_side === "challenger").length,
    opponent: completedRounds.filter((round) => round.winner_side === "opponent").length,
  };
}

function findCurrentRound(rounds: RoundRow[]) {
...
export async function GET(_request: Request, context: RouteContext) {
  try {
    ...
    if (!token) {
      return NextResponse.json({ ok: false, error: "Geçerli challenge bulunamadı." }, { status: 400 });
    }
    ...
    const { data: challengeData, error: challengeError } = await supabaseAdmin
      ...
      .select(`
        ...
        winner_side, completed_at, created_at, updated_at
      `)
    ...
    if (!challengeData) {
      return NextResponse.json({ ok: false, error: "Challenge bulunamadı." }, { status: 404 });
    }
    ...
    if (!isChallenger && !isOpponent) {
      return NextResponse.json({ ok: false, error: "Bu challenge'a erişim yetkin yok." }, { status: 403 });
    }

    const role: "challenger" | "opponent" = isChallenger ? "challenger" : "opponent";

    if (challenge.game_code !== "club_nation") {
      return NextResponse.json({ ok: false, error: "Bu challenge 1 Takım 1 Millet değil." }, { status: 409 });
    }
    ...
    const calculatedScore = calculateScore(rounds);
    const score = {
      challenger: Math.max(Number(challenge.challenger_score ?? 0), calculatedScore.challenger),
      opponent: Math.max(Number(challenge.opponent_score ?? 0), calculatedScore.opponent),
    };
    const calculatedWinner = score.challenger >= WIN_SCORE
      ? "challenger"
      : score.opponent >= WIN_SCORE
        ? "opponent"
        : null;
    const winnerSide = challenge.winner_side ?? calculatedWinner;
    const currentRound = findCurrentRound(rounds);
    const completedRoundCount = rounds.filter((round) => Boolean(round.completed_at)).length;
    const completed = challenge.status === "completed" || Boolean(challenge.completed_at) || Boolean(winnerSide);
    const prepared = rounds.length > 0;
    const readyToPlay = rounds.length === ROUND_COUNT && (challenge.status === "ready" || challenge.status === "playing");

    const myCurrentAnswer = currentRound
      ? role === "challenger"
        ? {
            answer: currentRound.challenger_answer,
            playerId: currentRound.challenger_answer_player_id,
            answeredAt: currentRound.challenger_answered_at,
          }
        : {
            answer: currentRound.opponent_answer,
            playerId: currentRound.opponent_answer_player_id,
            answeredAt: currentRound.opponent_answered_at,
          }
      : null;

    const opponentCurrentAnswer = currentRound
      ? role === "challenger"
        ? {
            answer: currentRound.opponent_answer,
            playerId: currentRound.opponent_answer_player_id,
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "1 Takım 1 Millet maç durumu okunamadı." },
      { status: 500 },
    );
  }
}
