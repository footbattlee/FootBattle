...
export async function GET(request: Request) {
  try {
    ...
    const peerScores = (peerRows ?? [])
      .map((row) => Number(row.server_score ?? 0))
      .filter((score) => Number.isFinite(score));
    const score = Math.max(0, Number(session.server_score ?? 0));
    const averageScore = peerScores.length
      ? Math.round(peerScores.reduce((sum, value) => sum + value, 0) / peerScores.length)
      : score;
    const belowOrEqual = peerScores.filter((value) => value <= score).length;
    const percentile = peerScores.length ? Math.round((belowOrEqual / peerScores.length) * 100) : 100;
    ...
    if (user) {
      const [{ data: profileRow }, { data: progressRow }, { data: unlockedRows }] = await Promise.all([
        supabaseAdmin
          .from("profiles")
        ...
        supabaseAdmin
          .from("user_progress")
          .select("xp, level, current_streak, best_streak")
          .eq("user_id", user.id)
          .maybeSingle(),
        supabaseAdmin
          ...
          .eq("user_id", user.id)
          .gte("unlocked_at", new Date(finishedAt.getTime() - 10_000).toISOString())
    ...
    return NextResponse.json({
      ...
      playerName: profile?.display_name ?? profile?.username ?? null,
      progression,
      recentAchievements,
    });
  } catch (error) {
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "Oyun sonucu hazırlanamadı." },
      { status: 500 },
    );
  }
}
