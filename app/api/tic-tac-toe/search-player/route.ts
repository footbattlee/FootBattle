...
export async function GET(request: Request) {
  try {
    ...
    if (query.length < MINIMUM_SEARCH_LENGTH) return NextResponse.json({ ok: true, minimumSearchLength: MINIMUM_SEARCH_LENGTH, players: [] });
    const normalizedQuery = normalizeSearch(query);
    if (!normalizedQuery) return NextResponse.json({ ok: true, minimumSearchLength: MINIMUM_SEARCH_LENGTH, players: [] });
    ...
    const { data, error } = await supabaseAdmin
      ...
      .limit(SEARCH_LIMIT);
    if (error) return NextResponse.json({ ok: false, error: "Oyuncular aranamadı." }, { status: 500 });
    ...
    console.error("TicTacToe search-player endpoint hatası:", error);
