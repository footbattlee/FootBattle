...
export async function POST(request: Request) {
  try {
    ...
    const { data: session, error: sessionError } = await supabaseAdmin
      ...
      .select("id, player_id, completed")
      .eq("id", sessionId)
      .maybeSingle();
    if (sessionError) return NextResponse.json({ ok: false, error: "Player Quiz oturumu kontrol edilemedi." }, { status: 500 });
    if (!session) return NextResponse.json({ ok: false, error: "Player Quiz oyunu bulunamadı." }, { status: 404 });
    if (session.completed) return NextResponse.json({ ok: false, error: "Bu Player Quiz oyunu zaten tamamlandı." }, { status: 409 });
    ...
    if (field === "nationality") {
      const { data, error } = await supabaseAdmin.from("guess_players").select("nationality").eq("player_id", playerId).maybeSingle();
      if (error || !data?.nationality) return NextResponse.json({ ok: false, error: "Milliyet kontrol edilemedi." }, { status: 500 });
      return NextResponse.json({ ok: true, field, correct: nationalitiesAreEquivalent(data.nationality, rawValue) });
    }
    ...
    console.error("Player Quiz guess endpoint hatası:", error);
