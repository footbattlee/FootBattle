...
async function identity() {
  ...
  if (user) {
    return { userId: user.id, voterKey: `u:${user.id}`, guestToken: null as string | null };
  }
...
export async function GET() {
  try {
    ...
    const { data: vote, error: voteError } = await supabaseAdmin
      .from("daily_faceoff_votes")
      .select("choice")
      .eq("faceoff_id", faceoff.id)
      .eq("voter_key", id.voterKey)
      .maybeSingle();
    ...
    const response = NextResponse.json({
      ...
      faceoff: {
        id: faceoff.id,
        date: faceoff.match_date,
        title: faceoff.title,
        category: faceoff.category,
        left: faceoff.left_name,
        right: faceoff.right_name,
      },
      voted,
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "Kapışma yüklenemedi." },
      { status: 500 },
    );
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const faceoff = await getTodayFaceoff();
    if (!faceoff) {
      return NextResponse.json({ ok: false, error: "Bugün için aktif kapışma yok." }, { status: 404 });
    }
    ...
    const response = NextResponse.json({
      ...
      alreadyVoted: Boolean(insertError?.code === "23505"),
      results,
    });
    return attachGuestCookie(response, id.guestToken);
  } catch (error) {
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "Oy kaydedilemedi." },
      { status: 500 },
    );
  }
}
