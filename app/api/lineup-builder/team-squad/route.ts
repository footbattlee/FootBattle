...
export async function GET(request: Request) {
  try {
    ...
    const players = (squadRows ?? [])
      .map((row) => {
        ...
        return {
          ...
          position: row.position ?? rawPlayer.position ?? null,
          ...
          popularityScore: rawPlayer.popularity_score === null ? null : Number(rawPlayer.popularity_score),
        };
      })
    ...
    const lineupData = Array.isArray(defaultLineup?.lineup_data)
      ? (defaultLineup.lineup_data as LineupSlot[]).map((slot) => ({
    ...
    return NextResponse.json({
      ...
      players,
      defaultLineup: {
        formationCode: defaultLineup?.formation_code ?? "4-2-3-1",
        slots: lineupData,
      },
    });
  } catch (error) {
