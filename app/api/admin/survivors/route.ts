...
function validate(input: SurvivorInput | null) {
  const titleTr = input?.titleTr?.trim() || input?.title?.trim() || "";
...
export async function GET() {
  ...
  const { data: sets, error } = await supabaseAdmin.from("survivor_sets").select("id, slug, title, description, title_tr, title_en, description_tr, description_en, kind, is_active, created_at, updated_at").order("created_at", { ascending: false });
  if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 500 });
  const ids = (sets ?? []).map((x) => x.id);
  const { data: entries, error: entriesError } = ids.length ? await supabaseAdmin.from("survivor_entries").select("id, set_id, slot, name, image_url, source_player_id").in("set_id", ids).order("slot") : { data: [], error: null };
  if (entriesError) return NextResponse.json({ ok: false, error: entriesError.message }, { status: 500 });
  const { data: results, error: resultsError } = ids.length ? await supabaseAdmin.from("survivor_results").select("set_id").in("set_id", ids) : { data: [], error: null };
  ...
  return NextResponse.json({ ok: true, items: (sets ?? []).map((set) => ({
    id: set.id, slug: set.slug,
    title: set.title_tr || set.title,
    description: set.description_tr ?? set.description ?? "",
    titleTr: set.title_tr || set.title,
    titleEn: set.title_en || "",
    descriptionTr: set.description_tr ?? set.description ?? "",
    descriptionEn: set.description_en || "",
    kind: set.kind, isActive: set.is_active, completions: completionCount.get(set.id) ?? 0,
    entries: (entries ?? []).filter((entry) => entry.set_id === set.id).map((entry) => ({ id: entry.id, slot: entry.slot, name: entry.name, imageUrl: entry.image_url, sourcePlayerId: entry.source_player_id })),
  })) });
}
...
export async function POST(request: Request) {
  ...
  const input = (await request.json().catch(() => null)) as SurvivorInput | null; const parsed = validate(input);
  if (!parsed.ok) return NextResponse.json({ ok: false, error: parsed.error }, { status: 400 });
  try {
    const slug = await uniqueSlug(parsed.titleTr);
    const { data: set, error } = await supabaseAdmin.from("survivor_sets").insert({ title: parsed.titleTr, description: parsed.descriptionTr, title_tr: parsed.titleTr, title_en: parsed.titleEn || null, description_tr: parsed.descriptionTr, description_en: parsed.descriptionEn || null, kind: parsed.kind, slug, is_active: input?.isActive ?? true }).select("id, slug").single();
    ...
    if (entriesError) { await supabaseAdmin.from("survivor_sets").delete().eq("id", set.id); throw entriesError; }
    return NextResponse.json({ ok: true, id: set.id, slug: set.slug });
  } catch (error) { return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "Survivor oluşturulamadı." }, { status: 500 }); }
}
...
export async function PATCH(request: Request) {
  ...
  const parsed = validate(input); if (!parsed.ok) return NextResponse.json({ ok: false, error: parsed.error }, { status: 400 });
  try {
    const slug = await uniqueSlug(parsed.titleTr, input.id);
    const { error } = await supabaseAdmin.from("survivor_sets").update({ title: parsed.titleTr, description: parsed.descriptionTr, title_tr: parsed.titleTr, title_en: parsed.titleEn || null, description_tr: parsed.descriptionTr, description_en: parsed.descriptionEn || null, kind: parsed.kind, slug, is_active: input.isActive ?? true, updated_at: new Date().toISOString() }).eq("id", input.id);
    if (error) throw error;
    const resolved = await resolveEntries(parsed.kind, parsed.entries);
    const { error: deleteError } = await supabaseAdmin.from("survivor_entries").delete().eq("set_id", input.id); if (deleteError) throw deleteError;
    const { error: entriesError } = await supabaseAdmin.from("survivor_entries").insert(resolved.map((entry) => ({ ...entry, set_id: input.id }))); if (entriesError) throw entriesError;
    return NextResponse.json({ ok: true, slug });
  } catch (error) { return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "Survivor güncellenemedi." }, { status: 500 }); }
}
